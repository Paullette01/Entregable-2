...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    @foreach($colaboradores as $colaborador)
        <div class="bg-white rounded-lg shadow-lg">
            <img src="{{ asset('uploads/' . $colaborador->imagen) }}" alt="Imagen del colaborador" class="w-full h-48 object-cover rounded-t-lg">
            <div class="p-4">
                <h3 class="text-xl font-medium mb-2">{{ $colaborador->nombre }}</h3>
                <p class="text-gray-600 mb-2">{{ $colaborador->especialidad }}</p>
                <p class="text-gray-600 mb-4">{{ $colaborador->numero_empleado }}</p>
